...
class DoctrineEventStore implements EventStoreInterface
{
    ...
    public function saveEvents(string $aggregateId, array $events, int $expectedVersion): void
    {
        ...
        try {
            ...
            if ($currentVersion !== $expectedVersion) {
                throw new \RuntimeException("Concurrency conflict: expected version {$expectedVersion}, got {$currentVersion}");
            }
    ...
    private function saveEvent(string $aggregateId, DomainEventInterface $event, int $version): void
    {
        ...
        if (false === $encoded) {
            throw new \RuntimeException(sprintf('Failed to encode event data for %s (aggregate %s): %s', $event->getEventType(), $aggregateId, json_last_error_msg()));
        }
    ...
    private function deserializeEvent(array $row): DomainEventInterface
    {
        ...
        if (null === $eventData && JSON_ERROR_NONE !== json_last_error()) {
            throw new \RuntimeException(sprintf('Failed to decode event data for event type %s: %s', $eventType, json_last_error_msg()));
        }
        ...
        if (!$constructor) {
            throw new \RuntimeException(sprintf('Event class %s has no constructor; cannot deserialize event data', $eventType));
        }
        ...
        foreach ($constructor->getParameters() as $param) {
            ...
            if (\App\Account\Domain\ValueObject\Money::class === $typeName) {
                ...
                if (!array_key_exists($paramName, $eventData)) {
                    throw new \RuntimeException(sprintf('Missing required key "%s" in event data for event type %s', $paramName, $eventType));
                }
                $args[] = new \App\User\Domain\ValueObject\Email($eventData[$paramName]);
            } elseif (null !== $typeName && enum_exists($typeName) && is_a($typeName, \BackedEnum::class, true)) {
                if (!array_key_exists($paramName, $eventData)) {
                    throw new \RuntimeException(sprintf('Missing required key "%s" in event data for event type %s', $paramName, $eventType));
                }
                ...
                if (!array_key_exists($paramName, $eventData) && !$param->isOptional()) {
                    throw new \RuntimeException(sprintf('Missing required key "%s" in event data for event type %s. Available keys: %s', $paramName, $eventType, implode(', ', array_keys($eventData ?? []))));
                }
